...
const coeficiente = (dataEntrou,dataSaiu,mes) =>{
    if(dataEntrou.toString().includes('/') || dataEntrou.toString().includes('-') || dataSaiu.toString().includes('/') || dataSaiu.toString().includes('-')){
        ...
        if(entrouEsseMes && !saiuEsseMes){
            ...
            return ((new Date(dataSaida).getDate() - new Date(dataEntrada).getDate())+1)/(new Date(calculoUltimoDia(ano,mes))).getDate()
        }if(continuaNaZiro){
            return 1
        } 
    }else{
