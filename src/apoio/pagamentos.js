...
const pagamentos = (mesInicio, mesFim, parcela1, modeloParcela2, baseComissoes, baseAssessor,apelido, dataEntrou, dataSaiu, baseReajuste,baseCobranca) => {
    let listPagamentos = []
    for(let i = mesInicio; i <= mesFim; i++){
        ...
        if(coeficiente){
            ...
            const resultParcela2 = Math.round(listarParcela2(reajusteModeloParcela2 || modeloParcela2, baseComissoes, baseAssessor, new Date().getFullYear(), i, apelido,baseCobranca)*100)/100
            listPagamentos.push({
