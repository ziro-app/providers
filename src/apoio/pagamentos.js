...
const pagamentos = (mesInicio, mesFim, parcela1, modeloParcela2, baseComissoes, baseAssessor,apelido, dataEntrou, dataSaiu, baseReajuste) => {
    ...
    for(let i = mesInicio; i <= mesFim; i++){
        ...
        const estaNaZiro = !entrouEsseMes && dataSaiu === '-' && stringDate(dataEntrou) <= new Date(new Date().getFullYear(),i) || dataSaiu !== '-' && !entrouEsseMes && stringDate(dataSaiu) <= new Date(new Date().getFullYear(), i) && stringDate(dataEntrou) >= new Date(new Date().getFullYear(),i)
        if(estaNaZiro){
