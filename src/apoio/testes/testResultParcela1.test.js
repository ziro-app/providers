...
const getEmployees = async () => {
    ...
    const arrayParcelas2Simuladas = parcelas2.map(item => item.map(item => item.parcela1))
    const simulados = arrayParcelas2Simuladas.flat()
    console.log('Simulado',simulados)
    console.log('Pago', basePago.map(item => item.parcela1))
    basePago.map(item => {
        if(!simulados.includes(item.parcela1)){
            console.log('\x1b[31m%s\x1b[0m',`O valor ${item.parcela1} pago no dia ${item.mes}/${item.ano} para a pessoa ${item.apelido} não foi simulado corretamente`)
            return `O valor ${item.parcela1} do dia ${item.mes}/${item.ano} não foi simulado corretamente`
        }
    })
}
