...
const lojista1 = (data, dataCalculo, mes, ano) => {
    ...
    const calculo = (novo, antigo) => {
        const calculo =  Number(novo)*0.005 + Number(antigo)*0.005
        return calculo
    }
    let resultado = []
    for(i=0; i < data.values.length; i++){
        if(data.values[i][4] == "Lojista1" && ano == 2019 && mes > 8 || data.values[i][4] == "Lojista1" && ano > 2019){
            ...
            resultado.push([data.values[i][0],data.values[i][1],data.values[i][2],data.values[i][3],0,data.values[i][4]])
        }else{
