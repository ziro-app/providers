...
const main = async (mes, ano) => {
    ...
    const listaBaseFuncionarios = await listarApelidos(dataBaseFuncionarios,mes,ano)
    const comissoesAssessores2 = await listarComissoesAssessores2(listaBaseFuncionarios,dataBaseAssessores,mes,ano)
    const comissoesProspeccao1 = await listarComissoesProspeccao(comissoesAssessores2,dataBaseResto,mes,ano)
    const comissaoCobranca1 = await listarCobranca1(comissoesProspeccao1,dataBaseResto,mes,ano)
    const comissaoLojista1 = await listarLojista1(comissaoCobranca1,dataBaseResto,mes,ano)
    try {
